...
class DashboardUi:
    ...
    def render(self):
        ...
        with ui.column().classes('w-full p-8'):
            ui.label(
                f'Bem-vindo, {usuario.get("nome", "Usuário")}!').classes('text-3xl font-bold mb-4')

            with ui.row().classes('mb-6'):
                ui.button('Ver Anúncios', on_click=lambda: ui.navigate.to(
                    '/anuncios')).props('color=primary')
                ui.button('Meus Favoritos', on_click=lambda: ui.navigate.to(
                    '/favoritos')).props('color=primary')

            with ui.card().classes('w-full max-w-2xl p-6'):
                ui.label('Informações do Usuário').classes(
